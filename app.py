...
def fetch_market_data(main_ticker, tickers_str):
    ...
    df_main.ta.bbands(length=20, std=2, append=True) # 加入布林通道
...
with st.spinner(f'📡 讀取 {market_name} 戰情室數據中...'):
    try:
        ...
        rsi, macd, macdh = latest['RSI_14'], latest['MACD_12_26_9'], latest['MACDh_12_26_9']
        bbu, bbl = latest['BBU_20_2.0'], latest['BBL_20_2.0']
        ...
        macd_status = "多頭延續" if macdh > 0 and macdh > prev['MACDh_12_26_9'] else "多頭降溫" if macdh > 0 else "空頭延續"
        ...
        with col_main:
            ...
            # 布林通道
            fig.add_trace(go.Scatter(x=df.index, y=df['BBU_20_2.0'], mode='lines', line=dict(color='rgba(255,255,255,0.2)', width=1, dash='dot'), name='上軌'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df.index, y=df['BBL_20_2.0'], mode='lines', line=dict(color='rgba(255,255,255,0.2)', width=1, dash='dot'), name='下軌', fill='tonexty', fillcolor='rgba(255,255,255,0.02)'), row=1, col=1)
